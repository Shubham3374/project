...
tp=open('Amazon\organizer.json','r+')
try:
    cp=json.load(tp)
    ...
    if c==1:
        ...
        if in1==1:
            ...
            if (len(F)*len(E)*len(P))==0 or '@' not in E or '.com' not in E:
                ...
                operations.Register('organizer','Amazon/members.json','Amazon/organizer.json',F,E,P)
            ...
            if (len(F)*len(E)*len(P))==0 or '@' not in E or '.com' not in E:
                ...
                operations.Register('member','Amazon/members.json','Amazon/organizer.json',F,E,P)
        ...
        if in1==1:
            ...
            print("Enter Password:")
            P=input()
            s=operations.Login('organizer','Amazon/members.json','Amazon/organizer.json',E,P)
            if s==False:
                print("Invalid Credentials")
                continue
            else:
                t=open('Amazon/organizer.json','r')
                cnt=json.load(t)
                t.close()
                n=""
                ...
                while True:
                    ...
                    if i1==1:
                        ...
                        if (len(Ev_Name)*len(Ev_ID)*len(St_dt)*len(En_dt)*len(str(Cap)))==0 or len(St_dt)!=10 or len(En_dt)!=10 or len(St_t)!=8 or len(En_t)!=8 or St_dt>En_dt or (St_dt==En_dt and St_t>En_t):
                            ...
                            operations.Create_Event(n,'Amazon/events.json',Ev_ID,Ev_Name,St_dt,St_t,En_dt,En_t,[],Cap,Cap)
                            print("Event created successfully")
                    elif i1==2:
                        ev_details=operations.View_Events(n,'Amazon/events.json')
                        if len(ev_details)==0:
